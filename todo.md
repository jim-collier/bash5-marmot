...
This is an easier way to brainstorm and prioritize tasks, before creating issues for them (if at all).

## Legend

Meaning of numbers for columns: "Pri" = Priority, "Eff" = Effort:

| Value | Meaning for positive fields (e.g. Priority)   | Meaning for 'negative' fields (e.g. Effort) |
| ---:  | :---                                          | :---                                        |
| 1     | Highest                                       | Easiest/Quickest
| 2     | Higher                                        | Easier/Quicker
| 3     | Normal/Default                                | Normal/Default
| 4     | Lower                                         | Harder/Slower
| 5     | Lowest                                        | Hardest/Slowest

## Staged

| Created  |Issue#|Score|Pri|Eff|Aff| Started  | by | Completed | Description | Notes
| :------  | :--- | --: |--:|--:|--:| :------  |:---| :-------- | :---------- | :----
...
| Created  |Issue#|Score|Pri|Eff|Aff| Started  | by | Completed | Description | Notes
| :------  | :--- | --: |--:|--:|--:| :------  |:---| :-------- | :---------- | :----
| 20250711 |      |     | 2 | 5 |   |          |    | 202605??  | Stop using Bash for shell scripting and use something like Ksh, [PowerShell core](https://github.com/PowerShell/PowerShell), [Deno](https://deno.com/) (TS/JS), [Xonsh](https://xon.sh/contents.html) (Python), [Groovy](https://www.groovy-lang.org/) (Java), [Nushell](https://www.nushell.sh/book/scripts.html), [YSH](https://oils.pub/ysh.html), or Go with shell helper modules. | After an thorough analysis, Bash v5 is really the only option. So this template will almost certainly always be relevant for certain system shell scripting tasks and tools that will run on any distro with few to no depencies. The change would be for bigger projects where absolutely zero external dependencies are essential, and/or where Bash performance is just too slow (e.g. long-running nested loops with floating-point math). Probably Go, definitely not Python. (See [comparison document](https://github.com/jim-collier/bash5-marmot/blob/main/Shell%20script%20comparison.md)). But I'm already using Go for such purposes, so ultimately this is kind of a moot task.
...
| Created  |Issue#|Score|Pri|Eff|Aff| Started  | by | Completed | Description | Notes
| :------  | :--- | --: |--:|--:|--:| :------  |:---| :-------- | :---------- | :----
| 20260309 |#1    |     | 1 |   | 4 | 20260517 | JC | 20260526  | Refactor monolithic template into versioned `source` modules, starting small.
| 20260309 |      |     | 1 |   | 5 | 20260518 | JC | 20260526  | Refactor first module's error-handling to not `exit` if top-level script is `source`d.
| 20260309 |      |     | 2 |   | 4 | 20260420 | JC | 20260520  | Create a build/git script that compresses a version, copies modules to path, and updates git.
| 20260309 |      |     | 3 |   | 2 | 20260520 | JC | 20260520  | Change license from GPLv3 to 2. | Actually to MIT.
| 20250711 |      |     | 4 |   | 3 | 20250713 | JC | 20250713  | Modify if necessary to be able to run "sourced" from one or more unit-testing files.
| 20250711 |      |     | 4 |   | 3 | 20250713 | JC | 20250713  | Move unit testing out of main template.
| 20250713 |      |     | 2 |   | 1 | 20250713 | JC | 20250713  | Rename most _f*() functions to just f*() for user-fliendliness.
| 20250715 |      |     |   | 2 | 2 |          |    |           | `set -u; set -o pipefail; shopt -s inherit_errexit \|\| true`, and re-run unit tests.
