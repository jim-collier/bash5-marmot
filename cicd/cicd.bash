...
fDoesDirHaveContents(){
	...
	[[ ! -d "${1}" ]]                                     && return ${ERRNUM_MSG_ALREADY_SHOWN}
	[[   -z "$(ls -1A "${1%%/}/" 2>/dev/null || true)" ]] && return ${ERRNUM_MSG_ALREADY_SHOWN}
	return 0; }
fBuildQuotedParams(){
...
fMustBeInPath(){
	...
	if [[ -z "${programToCheckForInPath}" ]]; then
		fThrowError "Not program specified."  "${FUNCNAME[0]}"; return ${ERRNUM_MSG_ALREADY_SHOWN}
	elif [[ -z "$(which ${programToCheckForInPath} 2>/dev/null || true)" ]]; then
		fThrowError "Not found in path: ${programToCheckForInPath}"; return ${ERRNUM_MSG_ALREADY_SHOWN}
	fi ;:;}
fIntroPromptToContinue(){
	...
	fPromptYN "Continue? (y|n): "  ||  { fEcho "User aborted."; return ${ERRNUM_MSG_ALREADY_SHOWN}; }; }
fPromptYN(){
	((doQuietly)) && return 0
	local promptStr="${1:-}"
...
_fTrap_Error_Soft(){   _ErrVal=1; false; return ${ERRNUM_MSG_ALREADY_SHOWN}; }
fThrowError(){
	...
	_ErrVal=1
	{ ((_doExitOnThrow)) && exit ${ERRNUM_MSG_ALREADY_SHOWN}; } || return ${ERRNUM_MSG_ALREADY_SHOWN}; }
